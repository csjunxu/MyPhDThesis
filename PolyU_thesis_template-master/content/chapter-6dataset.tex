...
Contrast to the proposed methods, the traditional denoising methods (e.g., LSSC, EPLL, NCSR, WNNM, and PGPD) designed for grey scale image would generate artifacts since they process each channel of the RGB image individually \cite{srcolor}. They cannot deal with the images which have different noise statistics in different channels as well as different local region. Hence, these methods would fail to process the realistic noisy images captured from real-world scenes. On the other hand, the discriminative based methods (e.g., MLP, CSF, TNRD, and DnCNN) are trained on paired clean and noisy images. These methods are largely depends on the training dataset they used in training, and would fail to achieve good performance when the noise in the testing images is different from the noise in the training images.
...
\\
\hline
PSNR & 37.40 & 36.29 &  &  &  &  & 38.07 & 37.71
\\
\hline
SSIM & 0.9526 & 0.9255 &  &  &  &  & 0.9615 & 0.9571
\\
\hline
Metric
&
