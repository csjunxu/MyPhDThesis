...
\textbf{More Camera Brand}: The RENOIR dataset \cite{RENOIR2014} contains only the Canon camera (T3i and S90) and the mobile brand Xiaomi Mi3 for image collection. This dataset only contains one camera brand and one mobile brand. The dataset in \cite{crosschannel2016} contains two camera brands including Canon (5D) and Nikon (D600 and D800). However, the images captured in our dataset are captured with 3 commonly used camera brands including Canon (5D), Nikon (D800), and Sony (A7II). 


\textbf{More Camera Setting}: The dataset \cite{crosschannel2016} only contains three different ISOs when capturing images with Nikon D800 and one ISO with Canon 5D and Nikon D600. However, in each of the camera we use in our dataset, we capture images with 5 different ISOs from low to high.

\textbf{More Realistic Scene}:
