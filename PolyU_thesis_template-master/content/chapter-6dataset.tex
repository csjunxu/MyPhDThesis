...
Another issue about the real noisy image denoising problem is that the noisy images with realistic noise has no ground truth, i.e., the corresponding noise-free image. Hence, the objective evaluation on the quality of denoised images is nearly impossible. However, the image quality assessment by subjective evaluation would be very biased due to the limited number of subjectives in the evaluation experiments and the control of the experimental process. The missing of ground truth will be very problematic we have no objective measurements to evaluate the image quality of the images denoised by different methods. 
