...
\textbf{More Camera Brands}: The RENOIR dataset \cite{RENOIR2014} contains only the Canon camera (T3i and S90) and the mobile brand Xiaomi Mi3 for image collection. This dataset only contains one camera brand and one mobile brand. The dataset in \cite{crosschannel2016} contains two camera brands including Canon (5D) and Nikon (D600 and D800). However, the images captured in our dataset are captured with 3 commonly used camera brands including Canon (5D), Nikon (D800), and Sony (A7II). 


\textbf{More Camera Settings}: In the RENOIR dataset \cite{RENOIR2014}, the reference images are all captured when the ISO=100. The ISO in noisy images are set as follows:  for Xiaomi Mi3, the ISO is set as 1600 or 3200; for Canon S90, the ISO is set as 640 or 1000; for Canon T3i, the ISO is set as 3200 or 6400. For all the cases except for the reference image of Canon S90, the exposure time are set as automatic. For Canon S90, the exposure time is set as 3.2 seconds. The dataset \cite{crosschannel2016} only contains three different ISOs when capturing images with Nikon D800 and one ISO with Canon 5D and Nikon D600. However, in each of the camera we use in our dataset, we capture images with 5 different ISOs, e.g., 800, 1600, 3200, 6400, and 12800. More ISOs from low to high values will help us 

\textbf{More Realistic Scenes}:

\textbf{Raw and sRGB Images for Comparison}:


In Figure \ref{fig6-2}, we show some realistic noisy images we captured in our new dataset. From which we can 
